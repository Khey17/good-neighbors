    ...
    async with pool.acquire() as conn:
        if role == "artist" and isinstance(body, ArtistProfileUpdate):
            await conn.execute(
                ...
                WHERE user_id = $8
                """,
                body.display_name, body.bio, body.category,
                body.skills, body.location, body.portfolio_url,
                body.instagram, user_id,
            )
            ...
            raise HTTPException(status_code=400, detail="Profile type doesn't match your account role.")
...
async def get_artist_profile(user_id: str):
    ...
    async with pool.acquire() as conn:
        row = await conn.fetchrow(
            """
            SELECT display_name, bio, category, skills,
                   location, portfolio_url, instagram, gig_count
...
async def _regenerate_artist_embedding(user_id: str, profile: ArtistProfileUpdate):
    ...
    try:
        ...
        async with pool.acquire() as conn:
            await conn.execute(
                "UPDATE artist_profiles SET embedding = $1 WHERE user_id = $2",
                embedding, user_id,
            )
